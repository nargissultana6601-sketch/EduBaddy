...
class VotingScreen extends StatefulWidget {
  const VotingScreen({super.key, required this.isCR});
...
class _PollCardState extends State<PollCard> {
  ...
  String _formatTimeLeft() {
    if (!hasEndDate || timeLeft == null) return "No expiry";
  ...
  Color _getTimerColor() {
    if (!hasEndDate || timeLeft == null) return Colors.grey;
    return timeLeft!.inHours < 24 ? Colors.orange : Colors.green;
  }

  Color _getTimerBackgroundColor() {
    if (!hasEndDate || timeLeft == null) return Colors.grey.shade100;
  ...
  Future<void> _castVote() async {
    ...
    try {
      ...
      final pollRef =
          FirebaseFirestore.instance.collection('polls').doc(widget.poll.id);
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        final currentVotes =
            List.from(freshPoll['votes'] ?? List.filled(options.length, 0));
  ...
  Widget build(BuildContext context) {
    ...
    final totalVotes =
        votes.fold<int>(0, (voteTotal, v) => voteTotal + (v as int));
    final createdAt =
        (widget.poll['createdAt'] as Timestamp?)?.toDate() ?? DateTime.now();
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: isExpired ? Colors.red.shade50 : _getTimerBackgroundColor(),
              ...
              Column(
                children: List.generate(options.length, (i) {
                  ...
                  return GestureDetector(
                    onTap: () =>
                        setState(() => selectedOption = options[i].toString()),
                    child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      LinearProgressIndicator(
                        ...
                        valueColor:
                            AlwaysStoppedAnimation(_getColorForIndex(i)),
                        minHeight: 8,
                ...
                if (!hasVoted && !isExpired)
                  Expanded(
                    child: ElevatedButton(
                      onPressed:
                          (selectedOption == null || isLoading) ? null : _castVote,
                      child: isLoading
                          ? const SizedBox(
                              height: 20,
                              width: 20,
...
class PastPollCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final endedManually = data?['endedManually'] as bool? ?? false;
    ...
    final winners =
        winnerIndices.map((i) => options[i].toString()).join(' & ');
